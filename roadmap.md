...
| ✅ Mergeado | #11 OpenAPI descriptions en endpoints | 2026-05-27 | `Query(...)` con description y example en todos los parámetros. summary y responses (200/400/404/500) en cada endpoint. Swagger UI autoexplicativo. |
...
| ✅ **Cerrado** | [#42](https://github.com/fedehofmann/oil_and_gas_mlops_pipeline/issues/42) Health checks | Implementados `GET /health/live` (liveness) y `GET /health/ready` (readiness: modelos cargados + parquet en disco). Fallback al modelo en disco pospuesto como mejora futura. Decisión #17 en README. |
...
| ✅ **Cerrado** | [#11](https://github.com/fedehofmann/oil_and_gas_mlops_pipeline/issues/11) OpenAPI descriptions en endpoints | `Query(...)` con `description` y `example` en todos los parámetros de `/api/v1/wells` y `/api/v1/forecast`. Se agregaron `summary` y tabla de `responses` (200/400/404/500) en cada endpoint. Swagger UI refleja los cambios automáticamente. |
